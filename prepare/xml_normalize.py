...
sub_dir2 = '/home/sathya/Dev/soundcloud-data/xml_normalized_final'

comment_ids = []

with open('comment_ids.txt', 'rb') as txt:
    for line in txt:
        comment_ids.append(int(line.strip()))
                    ...
                    if line in filename:
                        ...
                        for tag in tag_list:
                            ...
                            if '"' not in tag[0] and not(quoted):
                                tags = tags + remove_diacritic(tag) + ', '
                            else:
                                quoted = 1
                                tags = tags + remove_diacritic(tag) + ' '
                                if '"' in tag[len(tag)-1]:
                                    ...
                                    for comment in reversed(comments):
                                        if comment.timestamp:
                                            try:
                                                ...
                                                if len(comment.body) > 1 and 'http' not in comment.body and comment.user_id != track.user_id and comment.id in comment_ids:
                                                    comment_count += 1
